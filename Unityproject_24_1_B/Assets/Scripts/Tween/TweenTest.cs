...
public class TweenTest : MonoBehaviour
{
    ...
    {
        /*
        transform.DOMoveX(5, 0.5f);                                         //Tween을 통해서 오브젝트는 X축으로 2초동안 5만큼 이동
        transform.DORotate(new Vector3(0, 0, 180), 0.3f);                   //Tween 을 통해서 오브젝트는 Z축으로 0.3초 동안 180도 만큼 회전
        transform.DOScale(new Vector3(2,2,2), 2);                           //Tween을 통해서 오브젝트는 2초동안 2배로 커진다.
       

        Sequence sequence = DOTween.Sequence();                             // 시퀀스를 생성한다 (앞에 Tween이 끝나면 다음 트윈이 시작된다.)
        ...
        transform.DOMoveX(5, 2f).SetEase(Ease.OutBounce).OnComplete(DeactivateObject);                   // 이동창에 효과를 추가 시킬 수 있다 (Ease.OutBounce)
        transform.DOShakeRotation(2f, new Vector3(0, 0, 5), 10, 90);                                    //이동하는 동안 회전 충격 효과
        */
